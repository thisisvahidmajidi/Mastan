...
(function () {
  ...
  function getPerformance() { return getData().performance || {}; }
  ...
  var GLOSSARY = [
    ['ESG', 'ESG مخفف Environmental (محیط‌زیست)، Social (اجتماعی) و Governance (حکمرانی) است و شاخص‌هایی برای سنجش پایداری و مسئولیت‌پذیری سازمان در برابر محیط، جامعه و مدیریت شفاف.'],
    ['EFQM', 'مدل تعالی EFQM یک چارچوب خودارزیابی و بهبود سازمانی است که با ۵ توانمندساز و ۴ نتیجه، عملکرد سازمان را شواهد‌محور و در مقیاس ۰ تا ۱۰۰۰ امتیازدهی می‌کند.'],
    ['RADAR', 'منطق RADAR در EFQM شامل: Results (نتایج)، Approach (رویکرد)، Deployment (استقرار)، Assessment (ارزیابی) و Refinement (بهبود و یادگیری).'],
    ...
    ['PDCA', 'چرخه PDCA (Plan-Do-Check-Act) روش شناخته‌شده بهبود مستمر است: برنامه‌ریزی، اجرا، بررسی و اصلاح.'],
    ['SDGs', 'SDGs یا اهداف توسعه پایدار سازمان ملل؛ ۱۷ هدف جهانی برای رفع فقر، نابرابری و حفاظت از سیاره تا سال ۲۰۳۰.'],
    ...
    ['1:1', 'جلسه ۱:۱ یک گفت‌وگوی منظم و خصوصی میان سرپرست/مربی و کارمند است (معمولاً هفتگی) برای بازخورد، رشد و حل مسئله.'],
    ['افق ۱۴۱۰', 'افق ۱۴۱۰ اشاره به برنامه راهبردی منابع انسانی صنعت نفت دارد: «برند برتر کارفرمایی در ایران با شاخصه‌های توسعه‌یافتگی، جذابیت و بهره‌وری بالا در افق ۱۴۱۰».'],
    ['برند کارفرمایی', 'برند کارفرمایی یعنی تصویر و ارزشی که سازمان به‌عنوان محل کار در ذهن نیروی کار فعلی و آینده ایجاد می‌کند.'],
    ['توسعه‌یافتگی', 'در این پلتفرم یعنی سطح شایستگی، یادگیری و رشد منابع انسانی در مسیر برنامه راهبردی ۱۴۱۰.'],
    ['جذابیت و نگهداشت', 'جذابیت یعنی توانایی سازمان در جذب استعداد و نگهداشت یعنی کاهش ریزش و افزایش ماندگاری کارکنان.'],
    ['بهره‌وری بالا', 'بهره‌وری بالا یعنی تبدیل منابع انسانی، فرایندها و انرژی به نتایج بهتر با هزینه و زمان کمتر.'],
    ...
    ['هم‌آفرین', 'سازمان هم‌آفرین یا شبکه‌ای سازمانی است که تصمیم و نوآوری در تیم‌ها و شبکه‌ها توزیع می‌شود و روی شفافیت، اعتماد و همکاری تمرکز دارد.'],
    ['بوروکراتیک', 'سازمان بوروکراتیک دارای قوانین زیاد، سلسله‌مراتب سخت و تمرکز تصمیم در سطوح بالا است؛ در تغییرات سریع معمولاً کند است.'],
    ...
    ['وایزبورد', 'مدل شش‌جعبه‌ای وایزبورد (Weisbord, 1976) سازمان را از شش زاویه اهداف، ساختار، روابط، پاداش، رهبری و مکانیسم‌های کمکی تشخیص می‌دهد.'],
    ...
    ['نگرش شغلی', 'نگرش شغلی شامل رضایت، تعهد و سایر نگرش‌های کارکنان است که به عملکرد، رضایت مشتری و سودآوری متصل می‌شود.'],
    ['زنجیره ارزش', 'زنجیره ارزش در این مدل یعنی نگرش‌ها ← بهبود عملکرد کارکنان ← رضایت مشتریان ← سودآوری.'],
  ...
  function wrapTerms(text, terms) {
    ...
    while (cursor < text.length) {
      ...
      for (var i = 0; i < terms.length; i++) {
        var term = terms[i][0];
        var at = text.indexOf(term, cursor);
        if (at > -1 && (foundIndex === -1 || at < foundIndex || (at === foundIndex && term.length > terms[found][0].length))) {
          found = terms[i];
          foundIndex = at;
        }
      }
  ...
  function refreshReport() {
    ...
    setText('cr-report-perf-count', faNum(fmtNum(num(p.count))));
    setText('cr-report-perf-avg', faNum(fmtNum(num(p.average), 2)));
    setText('cr-report-perf-sbi', faNum(fmtNum(num(p.feedback_rate), 1)) + '٪');
    setText('cr-report-perf-oskar', faNum(fmtNum(num(p.oskar_rate), 1)) + '٪');
    setText('cr-report-perf-diagnosis', p.diagnosis || '');
    var perfBody = document.getElementById('cr-report-perf-tbody');
    if (perfBody && p.items) {
      var dims = p.dimensions || {};
      perfBody.innerHTML = p.items.map(function (it) {
  ...
  function refreshPerformance() {
    ...
    var items = p.items || [];
    var dims = p.dimensions || {};
    var summaryEl = document.getElementById('cr-perf-summary');
    ...
    if (summaryEl) {
      ...
      var oskar = num(p.oskar_rate);
      summaryEl.innerHTML =
        '<div class="cr-od-kpi"><span class="cr-od-kpi-label">کارکنان ارزیابی‌شده</span><span class="cr-od-kpi-value" data-fa-num>' + esc(p.count || 0) + '</span><small>دوره جاری</small></div>' +
        '<div class="cr-od-kpi"><span class="cr-od-kpi-label">میانگین عملکرد</span><span class="cr-od-kpi-value" style="color:' + esc((p.status && p.status.color) || '#2563eb') + ';" data-fa-num>' + esc(fmtNum(p.average, 2)) + '</span><small>از ۴</small></div>' +
        '<div class="cr-od-kpi"><span class="cr-od-kpi-label">بازخورد SBI</span><span class="cr-od-kpi-value" data-fa-num>' + esc(fmtNum(rate, 1)) + '٪</span><small>' + esc((p.feedback_given || 0) + ' جلسه') + '</small></div>' +
        '<div class="cr-od-kpi"><span class="cr-od-kpi-label">مربی‌گری OSKAR</span><span class="cr-od-kpi-value" data-fa-num>' + esc(fmtNum(oskar, 1)) + '٪</span><small>' + esc((p.oskar_used || 0) + ' جلسه') + '</small></div>' +
        '<div class="cr-od-kpi"><span class="cr-od-kpi-label">اثربخشی مربی‌گری</span><span class="cr-od-kpi-value" data-fa-num>' + esc(fmtNum(p.avg_effectiveness, 2)) + '</span><small>از ۴</small></div>';
      convertDigitsInside(summaryEl);
    }
    ...
    if (tbody) {
      tbody.innerHTML = items.map(function (it) {
        ...
        Object.keys(dims).forEach(function (k) {
          var sc = num(it.scores && it.scores[k]);
          parts.push('<td>' +
        ...
        return '<tr>' +
          ...
          '<td><span class="cr-od-perf-pill" style="color:' + esc((it.status && it.status.color) || '#333') + ';">' + esc((it.status && it.status.level) || '') + '</span></td>' +
          '<td><span class="cr-od-perf-overall" style="color:' + esc((it.status && it.status.color) || '#333') + ';">' + esc(fmtNum(it.overall, 2)) + '</span></td>' +
          parts.join('') +
    ...
    if (statusEl) {
      statusEl.textContent = p.diagnosis || '';
    }

    if (lowest) {
      var lows = p.low || [];
      if (!lows.length) {
        lowest.innerHTML = '<div class="cr-od-empty">در حال حاضر کارمند در آستانه افت عملکرد وجود ندارد؛ با پایش دوره‌ای، همین وضعیت را تثبیت کنید.</div>';
      } else {
        lowest.innerHTML = '<ul class="cr-od-perf-lowlist">' + lows.map(function (it) {
          return '<li><strong>' + esc(it.employee_name || '') + '</strong> — ' + esc(it.department || '') + ' (<span data-fa-num>' + esc(fmtNum(it.overall, 2)) + '</span>/۴) — ' + esc((it.status && it.status.likely) || '') + '</li>';
        }).join('') + '</ul>';
        convertDigitsInside(lowest);
      }
    }
  }

  function postSavePerformance(payload) {
    var form = document.getElementById('cr-perf-form');
    ...
    body.append('action', 'cr_od_save_performance');
  ...
  function bindPerformanceForm() {
    var form = document.getElementById('cr-perf-form');
    if (!form) { return; }
    var dims = (getPerformance() || {}).dimensions || {};
    ...
    form.addEventListener('submit', function (e) {
      ...
      ['employee_name', 'department', 'employee_role', 'supervisor_name', 'evaluator_role', 'period'].forEach(function (n) {
        ...
        if (!val && ['employee_name', 'department', 'period'].indexOf(n) !== -1) { missing.push(n); }
        payload[n] = val;
      });

      dimKeys.forEach(function (k) {
        var input = q('[name="score_' + k + '"]', form);
      ...
      payload.feedback_given = q('[name="feedback_given"]', form) ? !!q('[name="feedback_given"]', form).checked : false;
      payload.oskar_used = q('[name="oskar_used"]', form) ? !!q('[name="oskar_used"]', form).checked : false;
      payload.coaching_effectiveness = q('[name="coaching_effectiveness"]', form) ? clamp(parseFloat(q('[name="coaching_effectiveness"]', form).value) || 1, 1, 4) : 1;
      payload.growth_score = q('[name="growth_score"]', form) ? clamp(parseFloat(q('[name="growth_score"]', form).value) || 1, 1, 4) : 1;
      payload.notes = q('[name="notes"]', form) ? q('[name="notes"]', form).value.trim() : '';

      if (missing.length) {
        if (status) { status.textContent = 'لطفاً فیلدهای ضروری را کامل کنید: ' + esc(missing.slice(0, 2).join('، ')); status.className = 'cr-od-form-status err'; }
        return;
      }

      var btn = q('button[type=submit]', form);
      if (btn) { btn.disabled = true; btn.textContent = 'در حال ثبت...'; }
      if (status) { status.textContent = 'در حال محاسبه و ثبت...'; status.className = 'cr-od-form-status'; }

      postSavePerformance(payload)
        .then(function (res) {
          if (!res || res.success === false) {
            var msg = (res && res.data && res.data.message) ? res.data.message : 'خطا در ثبت ارزیابی عملکرد.';
            throw new Error(msg);
          }
  ...
  function bindReports() {
    ...
    if (csvBtn) {
      csvBtn.addEventListener('click', function () {
        ...
        rows.push([]);
        rows.push(['کارمند', 'واحد', 'امتیاز کل', 'وضعیت', 'SBI', 'OSKAR', 'اثربخشی', 'رشد']);
        (perf.items || []).forEach(function (it) {
          rows.push([
            it.employee_name || '',
            it.department || '',
            fmtNum(num(it.overall), 2),
            (it.status && it.status.level) || '',
            it.feedback_given ? 'دارد' : 'ندارد',
            it.oskar_used ? 'دارد' : 'ندارد',
            fmtNum(num(it.coaching_effectiveness), 1),
            fmtNum(num(it.growth_score), 1)
          ]);
        });
